...
public class AmazonSES {
	// This address must be verified with Amazon SES.
		final String FROM = "vaibvinesh@gmail.com";

		// The subject line for the email.
		final String SUBJECT = "One last step to complete your registration with my test app";
		...
		public void verifyEmail(UserDTO details) {
			AmazonSimpleEmailService client = AmazonSimpleEmailServiceClientBuilder.standard().withRegion(Regions.AP_SOUTH_1).build(); 
			
			String htmlBody = HTMLBODY.replace("$tokenValue", details.getEmailVerificationToken());
			String textBody = TEXTBODY.replace("$tokenValue", details.getEmailVerificationToken());
			
			SendEmailRequest request = new SendEmailRequest()
					.withDestination(new Destination().withToAddresses(details.getEmail()))
		...
		public boolean sendPasswordResetEmail(String firstName, String email, String token) {
			...
			String htmlBody = PASSWORD_RESET_HTMLBODY.replace("$tokenValue", token);
			htmlBody = htmlBody.replace("$firstName", firstName);
			
			String textBody = PASSWORD_RESET_TEXTBODY.replace("$tokenValue", token);
